const Invitation = () => {
    return (
      <div className="py-24 bg-brown-600 text-white text-center">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-semibold text-3xl mb-6 font-serif">
            Invite me to your restaurant. <br />
            Get exposure of millions food lovers.
          </h2>
          <p className="text-lg mb-10 font-serif">
            Lorem dui tincidunt nunc viverra morbi et maecenas quam adipiscing <br />
            integer amet eget blandit phasellus est natoque blandit facilisi eleifend.
          </p>
          <button className="bg-white text-brown-600 font-medium font-serif px-8 py-3 border-2 border-transparent hover:bg-transparent hover:border-white hover:text-white transition">
            LET'S TALK
          </button>
        </div>
      </div>
    );
  };
  
  export default Invitation;
  